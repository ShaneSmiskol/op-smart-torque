...
model_inputs = ['delta_desired', 'driver_torque', 'eps_torque', 'angle_steers', 'angle_offset', 'v_ego', 'time']
data = np.array([[math.degrees(sample[key] * 17.8) if key == 'delta_desired' else sample[key] for key in model_inputs] for sample in driving_data])  # todo: see if not converting to degrees will make the model learn quicker
# data = np.array([[sample[key] for key in model_inputs] for sample in driving_data])
...
data_sequences = []
for i in data_split:
    data_sequences += split_list(i, seq_len)  # todo: experiment with splitting list instead. lot less training data, but possibly less redundant data
...
for seq in data_sequences:
    if abs(np.interp(seq[-1][model_inputs.index('driver_torque')], [0, 1], scales['driver_torque'])) > 120:
        continue
    y_train.append([seq[-1][model_inputs.index('eps_torque')], seq[-1][model_inputs.index('angle_steers')]])
    if y_future != 0:
        seq = seq[:-y_future]
